...
const rewriteModulePath = async (src, oldImportIdentifier, newImportIdentifier) => {
...
const resolveModulePath = async (parentPath, importIdentifier) => {
    const parentUrl = pathToFileURL(parentPath).href;
    const targetPath = new URL(
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    await import.meta.resolve(importIdentifier, parentUrl)).pathname;
    if (!targetPath.includes('/'))
        return null;
    return targetPath;
};
const transformFile = async (pathBoundary, path, write) => {
    if (!path.startsWith(pathBoundary))
        return;
    if (handled.has(path))
        return;
    handled.add(path);
    ...
    for (const importSpecifier of imports) {
        ...
        const newModuleIdentifier = relativePath.startsWith('.')
            ? relativePath
            : `./${relativePath}`;
        src = await rewriteModulePath(src, oldImportIdentifier, newModuleIdentifier);
...
export async function transformTree(pathBoundary, path, write) {
    await init;
